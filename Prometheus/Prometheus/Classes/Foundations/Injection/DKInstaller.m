...
+ (BOOL)installHelperTool:(NSString *)executableLabel authorizationRef:(AuthorizationRef)authRef error:(NSError **)error {
  ...
  if (result == NO) {
    ...
    CFStringRef message = CFErrorCopyDescription(blessError);

    NSLog(@"an error occurred while installing %@ (domain: %@ (%@)) %@", executableLabel, errorDomain, [NSNumber numberWithLong:errorCode], message);
